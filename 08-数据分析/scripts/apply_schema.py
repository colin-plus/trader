...
SCHEMA = {
    ...
    "margin_factor": {
        "comment": "估值因子（静态，财报驱动）：每股收益/净资产/分红/股本，历史报告期逐期累积",
        "fk": "investable_asset",
        "columns": [
            ("code", "VARCHAR NOT NULL", "标的代码"),
            ("report_date", "DATE NOT NULL", "报告期（财报截止日）"),
            ("eps", "DOUBLE", "每股收益（元）"),
            ("bps", "DOUBLE", "每股净资产（元）"),
            ("dps", "DOUBLE", "每股分红（元，滚动12个月）"),
    ...
    "margin_daily": {
        "comment": "估值日快照（动态，每日盘后）：PE/PB/股息率，由因子+收盘价计算落库",
        "fk": "investable_asset",
        "columns": [
            ("code", "VARCHAR NOT NULL", "标的代码"),
            ("date", "DATE NOT NULL", "交易日"),
            ("pe", "DOUBLE", "市盈率 = 收盘价 ÷ 最新EPS"),
            ("pb", "DOUBLE", "市净率 = 收盘价 ÷ 最新BPS"),
            ("dividend_yield", "DOUBLE", "股息率（%） = DPS ÷ 收盘价 × 100"),
        ],
        "pk": ["code", "date"],
        "checks": [],
    },
    "margin_evaluation": {
        "comment": "安全边际评估记录（事件，人工决策留痕，一次评估一条）",
        "fk": "investable_asset",
        "columns": [
            ("id", "INTEGER NOT NULL", "评估序号（自增）"),
            ("code", "VARCHAR NOT NULL", "标的代码"),
            ("eval_date", "DATE NOT NULL", "评估日期"),
            ("price", "DOUBLE NOT NULL", "评估时价格（快照）"),
            ("pe", "DOUBLE", "评估时 PE（快照）"),
            ("pb", "DOUBLE", "评估时 PB（快照）"),
            ("dividend_yield", "DOUBLE", "评估时股息率（快照）"),
            ("pe_percentile", "DOUBLE", "评估时 PE 历史分位（快照）"),
